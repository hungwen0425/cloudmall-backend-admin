      ...
      <span class="custom-tree-node" slot-scope="{ node, data }">
        ...
        <span>
          ...
          <el-button
            ...
            type="text"
            size="mini"
            @click="() => removeCategory(node, data)">Delete</el-button>
        </span>
      </span>
    </el-tree>
...
export default {
  ...
  data() {
    return {
      ...
      // 宣告可否拖曳變數
      draggable: false,
      // 宣告托拽後收集的節點資料，最後調用 batchSave() 更新資料庫資料
      updateNodes: [],
      // 宣告節點最大深度變量
      maxLevel: 0,
      // 宣告對話框 title 值，在 append(data)、edit(data)方法中使用
      title: "",
      // 宣告對話框類型 (edit, add)，在 submitData()、append(data)、edit(data)方法中使用
      dialogType: "",
      // 宣告選單物件，綁定在對話框的 model <el-form :model="category">
      category: {
        name: "",
        parentCid: 0,
        catLevel: 0,
      ...
      // 宣告對話框 <el-dialog> 顯示的初始值，false : 關閉
      dialogVisible: false,
      // 宣告選單變數為 array
      menus: [],
      // 默認展開的 key 值
  ...
  methods: {
    ...
    getMenus() {
      this.$http({
        ...
        console.log("成功取得到商品選單資料...", data.data);
    ...
    allowDrop(draggingNode, dropNode, type) {
      // 1、被拖動的當前節點以及所在的父節點總層數不能大於 3
      // 1.1 被拖動的當前節點總層數
      console.log("allowDrop:", draggingNode, dropNode, type);
      this.countNodeLevel(draggingNode);
      // 當前正在拖動的節點+父節點所在的深度不大於 3 即可
